...
@Obanai.on_message(command(["gban", "globalban"], sudo_cmd=True))

async def gban(c: Obanai, m: Message):
...
async def ungban(c: Obanai, m: Message):
...
async def gban_list(_, m: Message):
    ...
    banfile = "Here are all the globally banned users!\n\n"
