...
async def approve_user(c: Obanai, m: Message):
...
async def disapprove_user(c: Obanai, m: Message):
...
@Obanai.on_message(command("approved") & admin_filter)
...
@Obanai.on_message(command("approval") & filters.group)

async def check_approval(c: Obanai, m: Message):
...
@Obanai.on_callback_query(filters.regex("^unapprove_all$"))
