...
@Obanai.on_message(filters.linked_channel)

async def antichanpin_cleanlinked(c: Obanai, m: Message):
...
@Obanai.on_message(filters.text & filters.group, group=5)
...
@Obanai.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)

async def gban_watcher(c: Obanai, m: Message):
...
Scanned by Team | FANTASTIC API ;)

Appeal [Here](https://t.me/TEAMSAMURAII)
...
@Obanai.on_message(filters.chat(BLACKLIST_CHATS))

async def bl_chats_watcher(c: Obanai, m: Message):
