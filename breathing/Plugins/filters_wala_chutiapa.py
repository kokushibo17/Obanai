...
from Powers.bot_class import LOGGER, Obanai
...
@Obanai.on_message(command("filters") & filters.group & ~filters.bot)
...
@Obanai.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@Obanai.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
...
@Obanai.on_callback_query(filters.regex("^rm_allfilters$"))
...
async def send_filter_reply(c: Obanai, m: Message, trigger: str):
...
@Obanai.on_message(filters.text & filters.group & ~filters.bot, group=69)

async def filters_watcher(c: Obanai, m: Message):
