from Messi.modules.disable import (
...
from Messi import dispatcher as d, LOGGER
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                LOGGER.debug(
                    f"[MESSICMD] Loaded handler {command} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(
                    f"[MESSICMD] Loaded handler {command} for function {func.__name__}"
                )
    ...
    def message(
        ...
        def _message(func):
            try:
                ...
                LOGGER.debug(
                    f"[MESSIMSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}"
                )
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                LOGGER.debug(
                    f"[MESSIMSG] Loaded filter pattern {pattern} for function {func.__name__}"
                )
...
messicmd = MessiHandler(d).command
messimsg = MessiHandler(d).message
messicallback = MessiHandler(d).callbackquery
