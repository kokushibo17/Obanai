...
import Messi.modules.sql.global_bans_sql as sql
from Messi.modules.sql.users_sql import get_user_com_chats
from Messi import (
    DEV_USERS,
    EVENT_LOGS,
    OWNER_ID,
...
from Messi.modules.helper_funcs.extraction import (
    extract_user,
    extract_user_and_text,
)
from Messi.modules.helper_funcs.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if not user_id:
        message.reply_text(
            "Ig that's not an user! or id is invalid!",
        )
        return
    ...
    if user_chat.type != "private":
        message.reply_text("That's not a user! uff")
        return
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if not reason:
            message.reply_text(
                "This jerk is already gbanned; I'd change the reason, but you haven't given me one...",
            )
            return
    ...
    try:
        bot.send_message(
            user_id,
            "#EVENT"
            "Sorry youre Gbanned from our bot! if its an mistake then appeal us at support chat."
...
def check_and_ban(update, user_id, should_message=True):
    ...
    if sw_ban:
        ...
        if should_message:
            update.effective_message.reply_text(
                f"<b>Alert</b>: this user may become harmful for your GC.\n"
                f"<code>*Removing from here*</code>.\n"
    ...
    if sql.is_user_gbanned(user_id):
        ...
        if should_message:
            text = (
                f"<b>Alert</b>: this user is globally banned by VOID\n"
                f"<code>*kicking them from here*</code>.\n"
                f"<b>Appeal chat</b>: @{SUPPORT_CHAT}\n"
...
def __stats__():
    return f"۞  ɴᴜᴍʙᴇʀ  ᴏғ  ɢʙᴀɴɴᴇᴅ  ᴜsᴇʀs : {sql.num_gbanned_users()}\n"
