...
from Messi import aiohttpsession as aiosession
from Messi.ex_plugins.dbfunctions import start_restart_stage
from Messi.utils.http import get, post


async def restart(m: Message):
    if m:
        await start_restart_stage(m.chat.id, m.message_id)
    execvp(executable, [executable, "-m", "Messi"])
...
def generate_captcha():
    ...
    height = 100
    correct_answer = ""
    font = ImageFont.truetype("Messi/resources/default.ttf", 55)
    file = f"Messi/resources/{randint(1000, 9999)}.jpg"
